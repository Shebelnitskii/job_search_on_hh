...
def creating_tables():
    ...
    try:
        ...
        if count_vacancy > 0 and count_employers > 0:
            ...
            limit_vacancy = int(input('Введите количество необходимых вакансий:\n'))
                ...
                with psycopg2.connect(
                    ...
                    with conn.cursor() as cur:
                        for row in hh_vacancies:
                            cur.execute(
                                "INSERT INTO vacancy (id, job_title, salary_from, salary_to, currency, description, area, employer, url, date) VALUES (%s, %s, %s, %s, %s, %s ,%s, %s, %s, %s)",
                                (row["id"], row["job_title"], row["salary_from"], row["salary_to"], row["currency"],
                                 row["description"], row["area"], row["employer"], row["url"], row["date"]))
                        for row in hh_employers:
                            cur.execute(
                                "INSERT INTO employers (id_vacancy, id_company, employer_name, description, site) VALUES (%s, %s, %s, %s, %s)",
                                (row["id_vacancy"], row["id_company"], row["employer_name"], row["description"], row["site"]))
            print('Таблицы заполнены')
    except:
        print('Неверные данные для БД')
